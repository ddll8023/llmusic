	...
	actions: {
		...
		updateCurrentTime(time) {
			...
			if (
				...
				// 只在播放中累加时间（前进方向）
				if (time > prevTime) {
					this.accumulatedPlayTime += time - prevTime;
				}

				// 当累积播放时间达到总时长的1/5时，触发计数
				const threshold = this.currentSong.duration / 5;
				if (this.accumulatedPlayTime >= threshold) {
					console.log(
						`歌曲 ${this.currentSong.title} 播放时间累计达到 ${this.accumulatedPlayTime.toFixed(1)}s (总时长 ${this.currentSong.duration}s 的 1/5 = ${threshold.toFixed(1)}s)，增加播放次数`
					);
