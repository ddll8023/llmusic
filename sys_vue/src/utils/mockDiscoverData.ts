...
export interface MockDiscoverData {
	recommendedSongs: MockSong[]
	hotPlaylists: MockPlaylist[]
	categories: MockCategory[]
	hotArtists: MockArtist[]
}
...
function generateSongs(count = 8): MockSong[] {
...
function generatePlaylists(count = 4): MockPlaylist[] {
	return Array.from({ length: count }, (_, i) => ({
		id: `playlist_${i + 1}`,
...
function generateArtists(count = 4): MockArtist[] {
	return Array.from({ length: count }, (_, i) => ({
		id: `artist_${i + 1}`,
...
export const mockDiscoverData: MockDiscoverData = {
...
/** 搜索模拟函数 */
export function mockSearch(keyword: string, type: 'all' | 'songs' | 'artists' | 'playlists' = 'all'): MockSearchResults {
	const results: MockSearchResults = { songs: [], artists: [], playlists: [] }
	if (!keyword) return results

	const searchTerm = keyword.toLowerCase()

	if (type === 'all' || type === 'songs') {
		results.songs = mockDiscoverData.recommendedSongs.filter(
			(song) =>
	...
	if (type === 'all' || type === 'playlists') {
		results.playlists = mockDiscoverData.hotPlaylists.filter(
			(playlist) =>
