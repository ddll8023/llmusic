...
onMounted(async () => {
  await qqmusicStore.loadLikedSongs();
});
...
const handlePageChange = (page: number) => {
  qqmusicStore.loadLikedSongs(page, qqmusicStore.likedPageSize);
};
</script>
...
<template>
  <div class="p-6 text-content-base h-full overflow-y-auto max-md:p-4">
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-xl font-bold max-md:text-lg">我喜欢的音乐</h2>
    </div>

    <LoadingSpinner v-if="qqmusicStore.likedLoading" text="加载中..." />

    <div v-else-if="qqmusicStore.likedSongs.length === 0" class="flex flex-col items-center justify-center py-16 text-content-secondary">
      <FAIcon name="heart" size="3x" color="secondary" />
      <p class="mt-4 text-sm">暂无喜欢的歌曲</p>
      <p class="mt-1 text-xs">登录 QQ 音乐后，收藏的歌曲将显示在这里</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="grid gap-2">
        ...
        <!-- 歌曲行 -->
        <div
          v-for="(song, index) in qqmusicStore.likedSongs" :key="song.songMid || index"
        ...
        >
          <div class="flex items-center justify-center text-content-tertiary text-xs">
            {{ index + 1 + (qqmusicStore.likedPage - 1) * qqmusicStore.likedPageSize }}
          </div>
      ...
      <!-- 分页 -->
      <div v-if="qqmusicStore.likedTotal > qqmusicStore.likedPageSize" class="flex items-center justify-center gap-2 mt-6">
        <CustomButton
          type="secondary" size="small"
          :disabled="qqmusicStore.likedPage <= 1"
          @click="handlePageChange(qqmusicStore.likedPage - 1)"
        >
          上一页
        </CustomButton>
        <span class="text-xs text-content-secondary">
          {{ qqmusicStore.likedPage }} / {{ Math.ceil(qqmusicStore.likedTotal / qqmusicStore.likedPageSize) }}
        </span>
        <CustomButton
          type="secondary" size="small"
          :disabled="qqmusicStore.likedPage * qqmusicStore.likedPageSize >= qqmusicStore.likedTotal"
          @click="handlePageChange(qqmusicStore.likedPage + 1)"
        >
          下一页
        </CustomButton>
