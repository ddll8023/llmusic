...
- **ContentHeader.vue** - 通用内容头部组件，支持标题、搜索、操作按钮
- **ContextMenu.vue** - 右键菜单组件，用于歌曲操作
- **DeleteConfirmDialog.vue** - 删除确认对话框，通用弹窗组件
- **FAIcon.vue** - FontAwesome 图标组件，全局使用
- **GlobalScanProgress.vue** - 全局扫描进度显示组件
- **Icon.vue** - SVG 图标组件
- **PlayerBar.vue** - 播放器控制栏，全局底部固定组件
- **Playlist.vue** - 播放列表组件，可在多处显示播放队列
- **SearchModal.vue** - 搜索模态框，全局搜索功能
- **SideBar.vue** - 侧边栏导航，全局布局组件
- **SongTable.vue** - 歌曲表格组件，可在多个页面显示歌曲列表
- **TagEditor.vue** - 标签编辑器，可在多处编辑歌曲元数据
- **TitleBar.vue** - 应用标题栏，全局顶部组件

#### 页面组件 (pages/)

特定页面的主要内容组件：

- **DiscoverMusic.vue** - 发现音乐页面主体
- **LyricPage.vue** - 歌词显示页面
- **MainContent.vue** - 音乐库主内容页面
- **MetadataManager.vue** - 元数据管理页面
- **PlaylistContent.vue** - 播放列表内容页面
- **PlaylistManage.vue** - 播放列表管理页面
- **Settings.vue** - 设置页面

#### 本地音乐功能组件 (local-music/)

针对本地音乐功能的专用组件：

- **LocalMusicHeader.vue** - 本地音乐页面专用头部组件
