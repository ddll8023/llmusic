...
// 标签字段映射 - 将标准字段名映射到不同格式的具体字段
const TAG_FIELD_MAPPING = {
  title: {
    mp3: 'TIT2',
    flac: 'TITLE',
    m4a: '©nam',
...
function validateTagChanges(tags) {
  ...
  if (tags.year && tags.year !== '') {
    ...
    if (isNaN(year) || year < 1900 || year > new Date().getFullYear() + 1) {
      errors.push('年份格式不正确，应为1900-' + (new Date().getFullYear() + 1) + '之间的数字');
    }
  }
...
async function updateSongTags(filePath, tags) {
  try {
    ...
    if (tags.artist) command.outputOptions(['-metadata', `artist=${tags.artist}`]);
