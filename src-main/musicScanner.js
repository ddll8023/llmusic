...
// 并行处理线程数，基于CPU核心数
const CONCURRENCY_LIMIT = Math.max(2, os.cpus().length - 1);
...
// 保留这些函数用于向后兼容或其他目的
async function findAudioFiles(dirPath, progressCallback = () => {}) {
	// 这个函数现在只是一个封装，实际扫描在Worker线程中进行
	console.warn("findAudioFiles被调用，但实际扫描在Worker线程中进行");
	return [];
}

async function parseAudioFile(filePath) {
	// 这个函数现在只是一个封装，实际解析在Worker线程中进行
	console.warn("parseAudioFile被调用，但实际解析在Worker线程中进行");
	return null;
}
