...
> 更新日期：2026-07-03
>
> - 2026-07-03：新增第三方下载源系统，BaseSongTable 操作列拆分官方/三方，设置页增加下载源管理
>
...
> - P2-1（已完成）：PlayerBar/LyricPage/SongTable/base.css 硬编码色值替换为 `var(--color-*)`
> - P2-2（已完成）：SideBar 玻璃背景变量化（`--color-surface-sidebar`）
> - P2-3（已完成）：GlassButton/ContextMenu/DiscoverMusic `transition-all` 限定属性
> - P2-4（已完成）：GlassButton/CustomButton 添加 `active:scale-95` 按压反馈
> - P3-1（已完成 → 2026-07-02 已撤销）：AmbientGlow 流动光晕背景已从 App.vue 移除使用（用户不需要彩色光晕效果），保留组件文件供后续复用
>
...
| 第三方下载源 | 独立于官方登录态的备用下载链路 | 多源管理、代理获取、设置页控制 |
...
- **搜索结果统一展示**，不对搜索来源做 Tab 区分。搜索统一走后端官方 API。

**关联模块**：QQ 音乐登录（需要登录凭证获取下载链接）、音乐下载（消费歌曲 URL）、音乐播放（在线试听）、第三方下载源（独立于登录的可选下载路径）
...
### 5.3b 第三方下载源（2026-07-03 新增）

**定位**：独立于 QQ 音乐登录的备用下载链路，通过第三方代理 API 获取音频链接。

**核心功能**：
- 多源管理：预置 vkeys.cn、xcvts.cn、317ak.cn、cyapi.top 四个第三方下载源
- 代理获取：前端 → 后端代理 → 第三方 API → 音频链接
- 链接验证：调用后端 HEAD 代理验证链接可访问性
- 源管理：设置页可开关/测试每个源，搜索栏可切换默认下载源

**设计边界**：
- **与官方下载完全平行独立**，不共享数据/链路/状态
- 不参与官方登录态，不依赖 QQ 音乐凭证
- 搜索结果中的三方播放走 HTML5 Audio，下载走 Electron IPC
- 不提供歌词、封面元数据——仅为裸音频下载

**操作列布局**（BaseSongTable.vue 在线模式）：

```
官方 [▶][⬇]  |  三方 [▶][⬇]
               ↑        ↑
           三方播放  三方下载（使用搜索栏选中的默认源）
```

**关联模块**：设置页（开关/测试源）、BaseSongTable（操作列）、Store（源管理+代理请求）

**开发关注点**：
- 第三方下载源可能随时失效，设置页提供测试按钮
- 同一首歌的 mid 不同平台不兼容，当前仅支持 QQ 音乐平台
- 下载前先验证链接可访问性，避免无意义的保存对话框
...
- **侧边栏收缩展开动画优化（2026-07-02）**：
  - SideBar 不再自控宽度，改为 `flex-1` 跟随父容器 `.sidebar-container`，消除双层过渡冲突
  - 父容器 `transition: width 0.15s ease-out` 为唯一过渡驱动，150ms 确保跟手
  - 侧边栏内边距由条件切换改为固定 `24px 20px`，不参与过渡，避免 layout 重排卡顿
  - 侧边栏背景 `--color-surface-sidebar` 由半透明 `rgba(26,26,26,.92)` 改为纯色 `#1a1a1a`，消除底层光晕穿透
  - 侧边栏与内容区的 resize handle（4px 分隔条）由 `bg-transparent` 改为 `bg-line-base`（`#282828`），防止彩色背景透过间隙可见
- **侧边栏状态管理优化（2026-07-02）**：
  - `uiStore.isSidebarCollapsed` getter 改为基于实时宽度（`currentDisplayWidth`），拖拽中响应正确
  - `toggleSidebarCollapse()` 同步更新 `tempSidebarWidth`，避免拖拽后折叠状态回滚
  - 新增 `previousExpandedWidth` 状态，折叠前保存展开宽度，展开时恢复而非硬编码 250px
...
- 歌词页面动画效果设置（淡入/滑动）
- 偏好数据的读写持久化
- **第三方下载源管理**：开关/测试每个下载源，重置默认配置

**设计边界**：
- 不做深度配置（如音质选择、主题色等）
- 配置项通过 Electron IPC 在主进程和渲染进程间同步
- 第三方下载源配置存储于 localStorage，不经过后端

**关联模块**：窗口与UI壳、第三方下载源 Store
...
### 8.8 使用第三方下载源下载歌曲（2026-07-03 新增）

1. 用户在搜索栏选择默认第三方下载源（如 vkeys.cn）
2. 用户点击歌曲行的三方播放/下载按钮
3. 前端 Store 调用后端代理 `POST /thirdparty/get-url`，传入源 URL 模板 + songMid
4. 后端代理请求第三方 API（如 vkeys.cn），返回音频链接
5. 下载时先调用 `POST /thirdparty/validate-url` 验证链接可访问性
6. 验证通过后 Electron 弹出保存对话框，开始下载
7. 若任一步失败，前端显示 Toast 提示（如「三方下载链接获取失败，请在设置中切换其他下载源」）

### 8.9 在设置页管理第三方下载源（2026-07-03 新增）

1. 用户进入设置页「第三方下载源管理」板块
2. 查看预置的 4 个下载源（vkeys.cn / xcvts.cn / 317ak.cn / cyapi.top）
3. 每个源有复选框开关 + 测试按钮
4. 点击「测试」调用后端 `POST /thirdparty/test-source`，返回延迟和可用性
5. 用户可「重置默认」恢复初始配置
